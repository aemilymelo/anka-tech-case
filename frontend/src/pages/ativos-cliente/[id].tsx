...
// Tipo de dado para representar um ativo financeiro
type Ativo = {
  id: number
  nome: string
  valor: number
...
// Validação do formulário de ativo com Zod
...
export default function AtivosPorClientePage() {
  ...
  // Busca os ativos vinculados ao cliente
  const { data: ativos, isLoading, error, refetch } = useQuery({
    ...
    enabled: !!id, // Executa somente se o ID estiver disponível
  })
  ...
  // Envia o formulário para criar ou atualizar um ativo
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          {ativos?.map((ativo) => (
            <tr key={ativo.id} className="hover:bg-gray-100">
              ...
              <td className="px-4 py-2 border-t flex space-x-2 justify-start">
                <Button onClick={() => handleEdit(ativo)} className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-md">
                  Editar
                </Button>
                <Button onClick={() => handleDelete(ativo.id)} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md">
                  Deletar
                </Button>
              </td>
            </tr>
      ...
      {/* Formulário para criar ou editar ativo */}
      <h2 className="text-2xl font-semibold text-[#161616] mt-8">
