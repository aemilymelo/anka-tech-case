...
export default function ClientesPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <TableBody>
          {clientes?.map((cliente) => (
            <TableRow key={cliente.id} className="hover:bg-gray-100">
              ...
              <TableCell className="px-4 py-2 border-t">{cliente.status ? "Ativo" : "Inativo"}</TableCell>
              <TableCell className="px-4 py-2 border-t">
                <Link href={`/ativos-cliente/${cliente.id}`} className="text-blue-600 hover:text-blue-800">Ver</Link>
              </TableCell>
              <TableCell className="px-4 py-2 border-t flex space-x-2">
                <Button onClick={() => handleEdit(cliente)} className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-md">Editar</Button>
                <Button onClick={() => handleDelete(cliente.id)} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md">Deletar</Button>
                <Button onClick={() => handleAddAtivo(cliente.id)} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-md">Adicionar Ativo</Button>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
      ...
      <h2 className="text-2xl font-semibold text-gray-800 mt-8">{editingClient ? "Editar Cliente" : "Cadastrar Novo Cliente"}</h2>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
        <div>
          <Input type="text" placeholder="Nome" {...register("nome")} className="w-full p-2 border border-gray-300 rounded-md" />
          {errors.nome && <p className="text-red-500">{errors.nome.message}</p>}
        </div>

        <div>
          <Input type="email" placeholder="Email" {...register("email")} className="w-full p-2 border border-gray-300 rounded-md" />
      ...
      {showAtivoForm && (
        <div className="mt-8">
          ...
          <form onSubmit={handleCadastrarAtivo} className="space-y-4">
            ...
            <Button type="submit" className="w-full py-3 bg-green-500 hover:bg-green-600 text-white rounded-md">
              Cadastrar Ativo
            </Button>
          </form>
        </div>
